...
function checkAutoUpdate() {
  ...
  var autoUpdate = Cookies.get('autoUpdate');
      ...
      for (var pool in stats) {
        ...
        if (typeof extraStratums !== 'undefined') {
          ...
          for (var stratum in extraStratums) {
            ...
            table.append('<thead class="text-custom"><th>Server:Port</th><th>Diff</th><th></th></thead>');
...
function getReadableHashRateString(hashrate, equihash, force2Places = false) {
  if (equihash) {
    hashrate = (hashrate * 2);
    if (hashrate < 1000000) {
...
function getReadableNetworkHashRateString(hashrate, equihash, force2Places = false) {
  if (equihash) {
    hashrate = (hashrate * 1000000);
    if (hashrate < 1000000)
      return '0 Sol';
...
function getBooleanCookieValue(name, defaultValue) {
  var cookie = Cookies.get(poolCoin+name);
  if (cookie !== undefined) {
      return cookie==='true';
  }
  return defaultValue;
...
function setBooleanCookieValue(name, value) {
  Cookies.set(poolCoin+name, value);
}

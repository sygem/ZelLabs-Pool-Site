    ...
    $.ajax({
      ...
      success: function (stats) {
        ...
        if (!chart || chart===null) {
            ...
            valueAxisHash.renderer.labels.template.adapter.add("text", (label, target, key) => {
                return $(window).width() > 500 ? getReadableHashRateString(target.dataItem.value, coinConfigData.algo==='equihash', false) : '';
            });
        ...
        if (lastChartDate == 0) {
          ...
          if (rigs.length > 1) {
            ...
            totalHashSeries.adapter.add('tooltipText',function(text, target) {
              ...
              return 'Total Hash: [bold]'+getReadableHashRateString(target.tooltipDataItem.dataContext["total"], coinConfigData.algo==='equihash')+'[/]';
            });
        ...
        for (var w in stats.workers) {
          ...
          tr.append(td);
          td = $('<td>'+getReadableHashRateString(calculateAverageHashrate(saneWorkerName, chartData, otherData), coinConfigData.algo==='equihash')+'</td>'); // Hashrate (Avg)
          tr.append(td);
        ...
        $("#currentHashrate").text(getReadableHashRateString(stats.totalHash, coinConfigData.algo==='equihash'));
        $("#5mAvgHashrate").text(getReadableHashRateString(stats.averages.avg5m, coinConfigData.algo==='equihash'));
        $("#1hAvgHashrate").text(getReadableHashRateString(stats.averages.avg1h, coinConfigData.algo==='equihash'));
        $("#3hAvgHashrate").text(getReadableHashRateString(stats.averages.avg3h, coinConfigData.algo==='equihash'));
        $("#24hAvgHashrate").text(getReadableHashRateString(stats.averages.avg24h, coinConfigData.algo==='equihash'));
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          var togglePercentageState = Cookies.get('togglepercentage');
          var toggleRewardState = Cookies.get('togglereward');
          var toggleEffortState = Cookies.get('toggleeffort');
          var toggleLuckState = Cookies.get('toggleluck');
          togglePercentageState = (togglePercentageState === undefined ? true : togglePercentageState === "true");
          toggleRewardState = (toggleRewardState === undefined ? true : toggleRewardState === "true");
          toggleEffortState = (toggleEffortState === undefined ? true : toggleEffortState === "true");
          toggleLuckState = (toggleLuckState === undefined ? true : toggleLuckState === "true");
          ...
          for (var pool in stats.pools) {
            ...
            for (var i = 0; i < blocks.length; i++) {
              ...
              if (blockData.sharePercentage && blockReward) {
                //console.log("???");
                td = $('<td class="percentage" style="'+(togglePercentageState ? '' : 'display: none;')+'">'+blockData.sharePercentage.toFixed(2)+'%</td>');
                tr.append(td);
                td = $('<td class="reward" style="'+(toggleRewardState ? '' : 'display: none;')+'">'+((blockData.sharePercentage*blockReward.blockReward/100)*(100-pooldata.poolFees)/100).toFixed(minerCoinPrecision)+' '+coin.toUpperCase()+'</td>');
                tr.append(td);
              } else {
                //console.log("nope");
  ...
  function addChartSeries(rig) {
    ...
    if (rigs.length==1) {
      rigSeries.adapter.add('tooltipText',function(text, target) {
        if (target == null || target.tooltipDataItem == null || target.tooltipDataItem.dataContext == null) return '';
        return 'Total Hash: [bold]'+getReadableHashRateString(target.tooltipDataItem.dataContext[rig.rigName], coinConfigData.algo==='equihash')+'[/]';
      });
    }
  ...
  function calculateAverageHashrate(worker, data, otherData) {
    ...
    avg = avg / count;
    //console.log("Average for "+worker+" is "+avg+" "+getReadableHashRateString(avg,coinConfigData.algo==='equihash')+" "+total+" "+count);
  	return avg;
  }
  ...
  function initColumn(name) {
    var toggleState = Cookies.get('toggle'+name);
  ...
  function toggleColumn(name) {
    ...
    Cookies.set('toggle'+name,toggleState);
    //console.log(name+" == "+toggleState);
    if (!toggleState) {

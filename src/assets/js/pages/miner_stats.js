  // Remove the wallet tab if it's not in use
  ...
  // Remove the loyalty program tab if it's not in use
  if (typeof loyaltyProgramEnabled == 'undefined') loyaltyProgramEnabled = false;
    ...
    coinConfigCheck = setInterval(() => {
      if (coinConfigData !== undefined) {
        if (coinConfigData.schema === 'solo') {
          log("Removing items not applicable to solo mining");
          $('.remove_solo').remove();
          $('.only_solo').each(function(index) {
            $(this).removeClass('only_solo');
          });
        } else {
          $('.only_solo').remove();
        }
  ...
  function updateWorkerStats() {
    ...
    $.ajax({
      ...
      success: function (stats) {
        ...
        // work out miner ttf (for solo mining pools)
        if (networkSols !== -1) {
        ...
        for (var w in stats.workers) {
          ...
          tr.append(td);
          td = $('<td>'+getReadableHashRateString(calculateAverageHashrate(saneWorkerName, chartData, otherData), coinConfigData.algo)+'</td>'); // Hashrate (Avg)
          tr.append(td);
    ...
    if (loyaltyProgramEnabled) {
      var loyaltyUrl = site+'/api/loyalty?address='+address+"&coin="+poolCoin;
      $.ajax({
        Method: 'GET',
        url: loyaltyUrl,
        dataType: 'json',
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for (var pool in stats.pools) {
          ...
          // work out miner ttf (for solo mining pools)
          if (minerSols !== -1) {
            updateMinerTTF(pooldata.poolStats.networkSols, minerSols)
          } else {
            networkSols = pooldata.poolStats.networkSols;
          }
        }
      }
    });
  ...
  function updateMinerTTF(networkSols, minerSols) {
    if (coinConfigData.schema === 'solo') {
      minerSols *= 2;
      minerSols /= 1000000;
      var luckDays = ((networkSols*1.2 / minerSols * coinConfigData.coinInfo.blockTime) / (24 * 60 * 60));
      if (luckDays < 0.04166667) {
        $("#ttf_miner").text((luckDays*24*60).toFixed(0)+" Minutes");
      } else if (luckDays < 1) {
        $("#ttf_miner").text((luckDays*24).toFixed(2)+" Hours");
      } else if (isFinite(luckDays)) {
        $("#ttf_miner").text(luckDays.toFixed(2)+" Days");
      } else {
        $("#ttf_miner").text(luckDays);
      }
    }
  }
  ...
  function calculateAverageHashrate(worker, data, otherData) {
    ...
    avg = avg / count;
    //console.log("Average for "+worker+" is "+avg+" "+getReadableNetworkHashRateString(avg/1000000,coinConfigData.algo)+" "+count);
  	return avg;
  }

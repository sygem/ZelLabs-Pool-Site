    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for (var pool in stats.pools) {
          ...
          var hashrate = (pooldata.hashrate) ? getReadableHashRateString(pooldata.hashrate, coinConfigData.algo) : pooldata.hashrateString;
          ...
          var networkSols = getReadableNetworkHashRateString(pooldata.poolStats.networkSols, coinConfigData.algo);
          ...
          if (luckDays < 0.0006944445) {
            $("#poolLuck").text((Number(luckDays)*24*60*60).toFixed(0)+" Seconds");
          } else if (luckDays < 0.04166667) {
            $("#poolLuck").text((Number(luckDays)*24*60).toFixed(0)+" Minutes");
          } else if (luckDays < 1) {
            $("#poolLuck").text((Number(luckDays)*24).toFixed(2)+" Hours");
          } else {
            //console.log(luckDays);
            $("#poolLuck").text(Number(luckDays).toFixed(2)+" Days");
          }
          ...
          $("#networkHashrate").text(networkSols);
          $("#networkDiff").text(networkDiff);
          $("#networkBlockHeight").text(networkBlocks);
          $("#networkBlockReward").text(pooldata.block.coininfo.blockReward+' '+symbol.toUpperCase());
          ...
          for (var block in blockTable.pendingblocks) {
            ...
            if (workername.length > 20) {
              workername = workername.substring(0,9)+' ... '+workername.substring(workername.length-10);
            }
            ...
            if (blockData.length > 7) {
              ...
              if (lbf !== -1 && ttf !== -1 && ttf !== Number.POSITIVE_INFINITY) {
                lbf /= 60;
            ...
            if (blockTable.confirms) {
              if (blockTable.confirms[blockData[0]]) {
                ...
                if (blockTable.confirms[blockData[0]] < numConfirms) {
                  if (blockTable.confirms[blockData[0]] < (numConfirms*0.15)) {
                    b = '  <div class="progress progress-lg mb-10">';
                    b += '    <div class="progress-bar progress-bar-info active progress-bar-striped progress-bar-animated" aria-valuemin="0" aria-valuemax="100" aria-valuenow="'+Math.round(blockTable.confirms[blockData[0]] * 100 / numConfirms)+'" style="width: '+Math.round(blockTable.confirms[blockData[0]] * 100 / numConfirms)+'%; font-size: 11px;" role="progressbar" data-toggle="tooltip" title="'+blockTable.confirms[blockData[0]]+' Confirmations"></div>';
                    b += '  </div>';
                  } else {
                    b = '  <div class="progress progress-lg mb-10">';
                    b += '    <div class="progress-bar progress-bar-info active progress-bar-striped progress-bar-animated" aria-valuemin="0" aria-valuemax="100" aria-valuenow="'+Math.round(blockTable.confirms[blockData[0]] * 100 / numConfirms)+'" style="width: '+Math.round(blockTable.confirms[blockData[0]] * 100 / numConfirms)+'%; font-size: 11px;" role="progressbar" data-toggle="tooltip" title="'+blockTable.confirms[blockData[0]]+' Confirmations"> '+blockTable.confirms[blockData[0]]+percentSymbol+'</div>';
                    b += '  </div>';
                  }
                  ...
                  b += '    <div class="progress-bar progress-bar-info active progress-bar-striped progress-bar-animated" aria-valuemin="0" aria-valuemax="100" aria-valuenow="100" style="width: 100%; font-size: 11px;" role="progressbar">'+numConfirms+percentSymbol+'</div>';
                  b += '  </div>';
                }
              } else {
          ...
          if (lastBlockTime === -1 || luckDays === 0) {
            ...
            var secondsSinceLastBlock = (Date.now() - lastBlockTime) / 1000;
            //console.log(secondsSinceLastBlock+" "+luckDays);
            $('#currentPoolLuck').text(((secondsSinceLastBlock / (luckDays*24*60*60))*100).toFixed(2)+'%');
          }

          // EFFORT CHART
          if (pooldata.effortDetails && !effortChart) {
          ...
          if (pooldata.effortDetails) {
            ...
            for (var i=0;i<pooldata.effortDetails.length;i++) {
              ...
              if (pooldata.effortDetails[i].numBlocks == 64 || pooldata.effortDetails[i].numBlocks == 128 || pooldata.effortDetails[i].numBlocks == 256 || pooldata.effortDetails[i].numBlocks == 512 || pooldata.effortDetails[i].numBlocks == 1024) {
                //console.log(pooldata.effortDetails[i]);
            ...
            $('#effortStats').removeClass('d-none').addClass('d-none');
          }
        ...
        success: function (data) {
            ...
            if (!chart || chart===null) {
                ...
                valueAxisHash.renderer.labels.template.adapter.add("text", (label, target, key) => {
                    return $(window).width() > 500 ? getReadableNetworkHashRateString(target.dataItem.value, coinConfigData.algo) : '';
                });
                ...
                poolHashSeries.adapter.add('tooltipText',function(text, target) {
                    ...
                    return '[bold]'+readableLongDate(target.tooltipDataItem.dataContext["time"])+'[/]\n'+
                           '- Pool Hash: '+getReadableNetworkHashRateString(target.tooltipDataItem.dataContext["poolHash"], coinConfigData.algo, true)+'\n'+
                           '- Network Hash: '+getReadableNetworkHashRateString(target.tooltipDataItem.dataContext["netHash"], coinConfigData.algo, true)+'\n'+
            ...
            if (lastChartDate == 0) {
              ...
              for (var i = 0, length = data.length; i < length; i++) {
                  ...
                  var diff = parseFloat(poolData.networkDiff);
                  var date = new Date(data[i].time*1000);
                  let obj = {"time":date,"poolHash":poolHash,"netHash":netHash,"diff":diff,"id":i};
                  d.push(obj);
